...
//              Xingyu Xie (xiexy17@mails.tsinghua.edu.cn)
//
...
// 2019: Letting the code run in 64-bit mode, change the type of PageNum
...
PF_BufferMgr::PF_BufferMgr(int _numPages) : hashTable(PF_HASH_TBL_SIZE)
{
    ...
    sprintf(psMessage, "Creating buffer manager. %d pages of size %d.\n",
            numPages, PF_PAGE_SIZE + (int)sizeof(PF_PageHdr));
    WriteLog(psMessage);
#endif
...
#ifdef PF_LOG
    char psMessage[100];
    sprintf(psMessage, "Looking for (%d,%lld).\n", fd, pageNum);
    WriteLog(psMessage);
#endif
...
#ifdef PF_LOG
    char psMessage[100];
    sprintf(psMessage, "Marking dirty (%d,%lld).\n", fd, pageNum);
    WriteLog(psMessage);
#endif
    ...
    while (slot != INVALID_SLOT)
    {
        ...
        if (bufTable[slot].fd == fd)
        {
...
#ifdef PF_LOG
            sprintf(psMessage, "Page (%lld) is in buffer manager.\n", bufTable[slot].pageNum);
            WriteLog(psMessage);
#endif
            ...
            else
            {
                ...
                {
#ifdef PF_LOG
                    sprintf(psMessage, "Page (%lld) is dirty\n", bufTable[slot].pageNum);
                    WriteLog(psMessage);
#endif
...
#ifdef PF_LOG
    char psMessage[100];
    sprintf(psMessage, "Forcing page %lld for (%d).\n", pageNum, fd);
    WriteLog(psMessage);
#endif
...
#ifdef PF_LOG
            sprintf(psMessage, "Page (%lld) is in buffer pool.\n", bufTable[slot].pageNum);
            WriteLog(psMessage);
#endif
...
#ifdef PF_LOG
    char psMessage[100];
    sprintf(psMessage, "Reading (%d,%lld).\n", fd, pageNum);
    WriteLog(psMessage);
#endif
...
#ifdef PF_LOG
    char psMessage[100];
    sprintf(psMessage, "Writing (%d,%lld).\n", fd, pageNum);
    WriteLog(psMessage);
#endif
...
RC PF_BufferMgr::AllocateBlock(char *&buffer)
{
    ...
    // pageNum is expected to be non-negative
