...
RC PF_FileHandle::GetThisPage(PageNum pageNum, PF_PageHandle &pageHandle) const
{
    ...
    {
        // printf("At GetThisPage(pageNum = %lld), IsValidPageNum(pageNum = %lld) = false\n", pageNum, pageNum);
...
#ifdef PF_LOG
    printf("GetThisPage(pageNum = %lld), pPageBuf->nextFree = %d, which is expected to be %d\n", pageNum, ((PF_PageHdr *)pPageBuf)->nextFree, PF_PAGE_USED);
#endif
...
{
#ifdef PF_LOG
    // printf("PF_filehandle::UnpinPage(pageNum = %lld)\n", pageNum);
#endif
